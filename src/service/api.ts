 ...
 */
export const fetchAssetsByCompany = async (
  companyId: string,
  page = 1,
  pageSize = 20
): Promise<Asset[] | null> => {
 ...
 * @returns {Promise<Company[] | null>} Array of companies or null if the request fails
 */
export const fetchMultipleCompanies = async (
  companyIds: string[]
): Promise<Company[] | null> => {
  return handleApiResponse(
 ...
 */
export const fetchCompanyWithCache = async (
  companyId: string
): Promise<Company | null> => {
  ...
  const company = await handleApiResponse(
